...
%Cargas archivos de voz
%[xn, fs] = audioread('Mensaje_Voz2.wav');
% [ch1, fs] = audioread('LetraA.wav');
% [ch2, fs] = audioread('LetraE.wav');
% [ch3, fs] = audioread('LetraI.wav');
% [ch4, fs] = audioread('Mensaje_Voz2.wav');
...
codeserial=reshape(code,[1,length(code)*bits]); %Informacion comprimida y cuantificada

figure,
plot(codeserial);
%title('Trama de bits que reune la información de los 4 canales a Tx');
title('CodeSerial');
xlabel('Muestra');
ylabel('Bit');
xlim([0 length(codeserial)-1]);
 
% Transmision OFDM
data=codeserial;
data_bits=length(codeserial); % Num. de bits 
...
% Modulacion
bpsk_data=pskmod(data,2); %BPSK
%bpsk_data=pskmod(data,4); %QPSK
%bpsk_data=qammod(data,4); %4QAM
%bpsk_data=qammod(data,16); %16QAM

figure,
stem(abs(bpsk_data));
title('Modulación BPSK ');
scatterplot(bpsk_data);%constelacion
grid on;

% Convertir serie datos en paralelo
Flujo=reshape(bpsk_data,data_bits/M,M);
Sc1 = Flujo(:,1);
Sc2 = Flujo(:,2);
Sc3 = Flujo(:,3);
Sc4 = Flujo(:,4);
...
xlabel('Tiempo');
ylabel('Amplitud');
title('Señal OFDM antes del canal AWGN');grid on;
...
xlabel('Tiempo');
ylabel('Amplitud');
title('Señal OFDM despues del Canal AWGN');grid on;
...
demod_data = pskdemod(rx_serial,2);%psk
%demod_data = pskdemod(rx_serial,4);%QPSK
%demod_data = qamdemod(rx_serial,4);%4QAM
%demod_data = qamdemod(rx_serial,16);%16QAM

figure,                 %COMPARACION TX y RX
stem(demod_data,'rx');
grid on;
xlabel('Puntos de Datos');
ylabel('Amplitud');
title('Comparacion Señal Rx con Tx')
hold on
stem(data,'bx'); 
legend('Rx','Tx');
...
audiowrite('OFDM_BPSK_LetraA.wav',msg1, fs);
audiowrite('OFDM_BPSK_LetraE.wav',msg2, fs);
audiowrite('OFDM_BPSK_LetraU.wav',msg3, fs);
audiowrite('OFDM_BPSK_MensajeVoz.wav',msg4, fs);
